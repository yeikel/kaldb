...
public abstract class InternalOrder extends BucketOrder {
    ...
    public static class Aggregation extends InternalOrder {
        ...
        public <T extends MultiBucketsAggregation.Bucket> Comparator<T> partiallyBuiltBucketComparator(ToLongFunction<T> ordinalReader, Aggregator aggregator) {
            try {
                Aggregator.BucketComparator bucketComparator = path.bucketComparator(aggregator, order);
...
//        }
        ...
        @Override
        public Comparator<MultiBucketsAggregation.Bucket> comparator() {
            List<Comparator<MultiBucketsAggregation.Bucket>> comparators = orderElements.stream().map(BucketOrder::comparator).collect(toList());
            return (lhs, rhs) -> {
                Iterator<Comparator<MultiBucketsAggregation.Bucket>> itr = comparators.iterator();
                int result;
                do {
                    result = itr.next().compare(lhs, rhs);
                } while (result == 0 && itr.hasNext());
    ...
    private static class SimpleOrder extends InternalOrder {
        ...
        SimpleOrder(byte id, String key, SortOrder order, Comparator<MultiBucketsAggregation.Bucket> comparator) {
            this.id = id;
            this.key = key;
            this.order = order;
