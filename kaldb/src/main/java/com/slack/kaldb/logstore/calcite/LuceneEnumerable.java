 ...
 *
 * <p>The class holds all the necessary logic to extract data from a Lucene index, transform it to
 * rows using Calcite's internal representation, and construct Enumerator objects, which can be
 * consumed by Calcite's {@link org.apache.calcite.adapter.enumerable.EnumerableRel} (physical)
 * operators.
 *
 * <p>Performance note: The class could be optimized to not fetch in-memory all the results from the
 * index by exploiting some additional Lucene APIs such as {@link
 * IndexSearcher#searchAfter(ScoreDoc, Query, int)}. This is not done in the current implementation
...
public class LuceneEnumerable extends AbstractEnumerable<Object[]> {
  ...
  public LuceneEnumerable(
      String indexPath, LinkedHashMap<String, SqlTypeName> fields, String query) {
