...
public class JavaDateMathParser implements DateMathParser {
    ...
    public Instant parse(String text, LongSupplier now, boolean roundUpProperty, ZoneId timeZone) {
        ...
        if (text.startsWith("now")) {
            try {
                ...
                throw new IllegalArgumentException("could not read the current timestamp", e);
            }
        ...
        for (int i = 0; i < mathString.length();) {
            ...
            if (c == '/') {
                ...
                if (c == '+') {
                    ...
                    throw new IllegalArgumentException(String.format("operator not supported for date math [%s]", mathString));
                }
            }

            if (i >= mathString.length()) {
                throw new IllegalArgumentException(String.format("truncated date math [%s]", mathString));
            }
            ...
            if (round) {
                if (num != 1) {
                    throw new IllegalArgumentException(String.format("rounding `/` can only be used on single unit types [%s]", mathString));
                }
            }
            ...
            switch (unit) {
                ...
                case 's':
                    ...
                    break;
                default:
                    throw new IllegalArgumentException(String.format("unit [%s] not supported for date math [%s]", unit, mathString));
            }
    ...
    private Instant parseDateTime(String value, ZoneId timeZone, boolean roundUpIfNoTime) {
        ...
        try {
            ...
            throw new IllegalArgumentException(String.format("failed to parse date field [%s] with format [%s]: [%s]", e, value, format), e);
        }
    }
}
