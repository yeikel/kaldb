 ...
 * slot is usually an ordinal into a set of values, e.g. tracking a count frequency <em>per
 * term</em>. Sometimes there doesn't need to be a slot distinction, in which case there is just one
 * nominal slot.
 */
public abstract class SlotAcc implements Closeable {
  ...
  /**
   * NOTE: this currently detects when it is being reused and calls resetIterators by comparing
   * reader ords with previous calls to setNextReader. For this reason, current users must call
   * setNextReader in segment order. Failure to do so will cause worse performance.
   */
  public void setNextReader(LeafReaderContext readerContext) throws IOException {
    ...
    if (lastReaderContext == null || lastReaderContext.ord >= currentReaderContext.ord) {
      // if we went backwards (or reused) a reader, we need to reset iterators that can be used only
      // once.
      resetIterators();
    }
  }
   ...
   * default impl of {@link #collect(DocSet, int, IntFunction)} but it's also neccessary if this
   * accumulator is used for sorting.
   *
   ...
   *     about the current slot -- the {@link IntFunction} is only garunteed to be valid for the
   *     current slot, and the {@link SlotContext} returned is only valid for the duration of the
   *     <code>collect()</code> call.
   */
  public abstract void collect(int doc, int slot, IntFunction<SlotContext> slotContext)
      throws IOException;

  /**
   * Bulk collection of all documents in a slot. The default implementation calls {@link
   * #collect(int, int, IntFunction)}
   *
  ...
  public int collect(DocSet docs, int slot, IntFunction<SlotContext> slotContext)
      throws IOException {
    int count = 0;
    IndexSearcher searcher = fcontext.searcher;
  ...
  public abstract static class DoubleFuncSlotAcc extends FuncSlotAcc {
    ...
    public DoubleFuncSlotAcc(
        ValueSource values, FacetContext fcontext, int numSlots, double initialValue) {
